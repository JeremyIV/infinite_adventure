from flask import Flask, session, redirect, url_for, request, send_file, send_from_directory
...
def new_state(state, player_prompt):
    ...
    state['objects'] = sorted((set(state['objects']) | set(new_objects)) - set(data['remove_objects']))

    # update inventory
    state['inventory'] = sorted((set(state['inventory']) | set(data['new_items'])) - set(data['remove_items']))
